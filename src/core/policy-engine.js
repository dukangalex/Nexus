...
function cidrContains(actual, expected) {
  // CIDR matching is deliberately delegated to a normalized fact supplied by the caller.
  // This engine does not guess IP ranges. `facts.ip_cidr` may contain the matched CIDR labels.
  return includesMatch(expected, actual);
}

function matchLogical(logical, facts) {
  if (!logical || typeof logical !== "object") return false;
  const mode = normalize(logical.mode || "and");
  const results = Array.isArray(logical.rules)
    ? logical.rules.map((rule) => matchObject(rule, facts))
    : [];
...
export function resolveRoutingPolicy(policyInput, facts = {}, options = {}) {
  ...
  if (policy.mode === "global_bypass") {
    return { ok: true, action: { type: "bypass", target: options.bypassTarget || "direct" }, rule: null };
  }
  ...
  for (const rule of ordered) {
    if (matchesRoutingRule(rule, facts)) {
      return { ok: true, action: structuredClone(rule.action), rule };
    }
  }

  if (options.failClosed === true) {
    return { ok: true, action: { type: "reject" }, rule: null, reason: "no rule matched; fail-closed" };
  }
